...
Use `doc-collector` as the bootstrap path for a new project that already has code after `./scripts/setup.sh` has installed skills and initialized CodeGraph. If the user provides product notes rather than an existing codebase to scan, use `documenter` instead.

## Source Exploration

Use CodeGraph first, or another code-exploration plugin the project prefers. If the repository has no `.codegraph/` index and no such plugin, stop and tell the user to run project setup or `codegraph init`; do not infer a full feature map from ad hoc file search alone.
...
- Do not create implementation `task` tickets unless the user also asks to implement. doc-collector is documentation discovery, not implementation planning.
- After doc-collector creates or updates the `group -> feature[]` tree, stop for user review. When the user later asks to implement one of those tickets, switch to `task-implementer` so task planning uses a fresh source scan and the approval gate.
