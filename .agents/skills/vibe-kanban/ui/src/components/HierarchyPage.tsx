...
function renderTreeNode(
  ...
  const row = (
    <div
      className={`group grid min-h-11 grid-cols-[minmax(0,1fr)_auto_minmax(92px,132px)] items-center gap-3 rounded-xl border border-transparent px-3 py-2 transition hover:border-neutral-200 hover:bg-white hover:shadow-sm max-lg:grid-cols-1 max-lg:items-start ${isMatch ? "" : "opacity-55"}`}
    >
      <button
        className="flex min-w-0 items-center gap-2 border-0 bg-transparent p-0 text-left disabled:cursor-default"
        type="button"
        disabled={!hasTicket}
        onClick={() => node.ticket && onOpen(node.ticket.id)}
      >
        <span
          className={`grid size-7 shrink-0 place-items-center rounded-lg border text-[10px] font-extrabold ${badgeToneClass(tone)}`}
        >
          {markerFor(node)}
        </span>
        <span className={`${ticketCodeClass} min-w-[74px] shrink-0`}>
          {node.code}
        </span>
        <span className="min-w-0 truncate text-sm font-semibold leading-5 text-neutral-800 group-hover:text-neutral-950">
          {node.title}
        </span>
      </button>
      <div className="flex flex-wrap gap-1 max-lg:pl-9">
        {type ? (
          <Badge tone={typeTone[type]}>{formatType(type)}</Badge>
        ) : (
          <Badge tone={tone}>{node.typeLabel}</Badge>
        )}
        {kind ? <Badge tone={kindTone[kind]}>{formatKind(kind)}</Badge> : null}
        {status ? (
          <Badge tone={statusTone[status]}>{formatStatus(status)}</Badge>
        ) : (
          <Badge tone="violet">{node.statusLabel}</Badge>
        )}
        {hasTicket ? (
          <Badge tone={node.reviewed ? "emerald" : "orange"}>
            {node.reviewed ? "Approved" : "Needs review"}
          </Badge>
        ) : null}
      </div>
      <div className="grid grid-cols-[34px_minmax(0,1fr)] items-center gap-2 max-lg:w-full max-lg:pl-9">
        ...
        <ProgressBar
          ...
          tone={status && statusTone[status] ? statusTone[status] : "violet"}
        />
      </div>
    </div>
  );
      ...
      {children.length ? (
        <details className="group/tree" open>
          <summary className="grid cursor-pointer list-none grid-cols-[18px_minmax(0,1fr)] items-center gap-1 rounded-xl marker:hidden [&::-webkit-details-marker]:hidden">
            <span className="text-center text-xs font-black text-neutral-400 transition group-open/tree:rotate-90">
              +
            </span>
            {row}
          </summary>
          <ol className="ml-[27px] mt-1 grid list-none gap-1 border-l border-neutral-200 pl-3">
            {children.map((child) =>
              renderTreeNode(child, depth + 1, visibleIds, onOpen),
            )}
          </ol>
        </details>
      ) : (
        <div className="grid grid-cols-[18px_minmax(0,1fr)] items-center gap-1">
          <span className="text-center text-xs font-black text-neutral-300">
            -
          </span>
          {row}
        </div>
    ...
    <section className={`${panelClass} mb-4 overflow-hidden p-0`}>
      <header className="flex items-center justify-between gap-3 border-b border-neutral-200 bg-white px-4 py-3">
        <div>
          ...
          <p className="m-0 text-xs leading-5 text-neutral-500">
            group folders, use case folders, and task leaves from the current
            filters.
          </p>
        </div>
        <strong className="shrink-0 text-xs font-extrabold uppercase text-violet-700">
          {tickets.length} shown
        </strong>
      </header>
      {topLevel.length ? (
        <ol className="m-0 grid list-none gap-1 bg-neutral-50/70 p-3">
          {topLevel.map((node) => renderTreeNode(node, 0, visibleIds, onOpen))}
        </ol>
      ) : (
