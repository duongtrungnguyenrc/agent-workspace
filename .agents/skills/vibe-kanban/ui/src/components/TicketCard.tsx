...
import { borderToneClass, buttonClass, buttonTone, statusTone, ticketCodeClass, typeTone } from "../lib/styles";
...
export function TicketCard({ ticket, tickets, onOpen, onAction }: TicketCardProps) {
  ...
  return (
    <button
      className={`grid min-w-0 gap-3 rounded-2xl border border-l-4 bg-white p-3 text-left shadow-[0_1px_2px_rgb(15_23_42/0.04)] transition hover:-translate-y-0.5 hover:shadow-lg hover:shadow-neutral-950/10 active:translate-y-px ${dragging ? "scale-[0.98] opacity-60" : ""} ${borderToneClass(typeTone[ticket.type])}`}
      draggable
      onDragStart={(event) => {
        setDragging(true);
    ...
    >
      <header className="flex min-w-0 items-start justify-between gap-2">
        <div className="grid min-w-0 gap-1.5">
          ...
          <strong className="line-clamp-2 break-words text-[15px] font-semibold leading-5 text-neutral-950">{ticket.title}</strong>
        </div>
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="shrink-0 text-xs font-bold text-neutral-500">{progress}%</span>
      </div>
      ...
      <footer className="flex min-w-0 flex-wrap items-center justify-between gap-2">
        ...
        <span className="min-w-0 truncate text-xs font-semibold text-neutral-500">{ticket.branch || "No branch"}</span>
      </footer>
      {needsApproval ? (
        <button
      ...
      <p className="m-0 rounded-lg bg-white/60 px-2 py-1 truncate text-xs font-medium text-neutral-500">{parentLabel(ticket, tickets)}</p>
    </button>
  );
}
