...
import { formatStatus, formatType, progressFor, ticketCode } from "../lib/format";
import { Markdown } from "../lib/markdown";
import { buttonClass, buttonTone, fieldClass, inputClass, mutedTextClass, panelClass, secondaryButtonClass, selectClass, statusTone, textareaClass, ticketCodeClass, typeTone } from "../lib/styles";
import type { TicketDetail, TicketListItem, TicketStatus, TicketType } from "../types/kanban";
...
function eligibleParents(type: TicketType, tickets: TicketListItem[], excludeId: number) {
  if (type === "use_case") return tickets.filter((ticket) => ticket.type === "US" && ticket.id !== excludeId);
  if (type === "task") return tickets.filter((ticket) => ["US", "use_case", "uat_feedback", "qc_feedback"].includes(ticket.type) && ticket.id !== excludeId);
  if (type === "uat_feedback" || type === "qc_feedback") return tickets.filter((ticket) => ticket.type === "US" && ticket.id !== excludeId);
  return [];
}

function Property({ label, value }: { label: string; value?: string | number | null }) {
  return (
    <div className="grid grid-cols-[110px_minmax(0,1fr)] gap-3 border-b border-neutral-100 py-2 last:border-b-0 max-sm:grid-cols-1 max-sm:gap-1">
      ...
      <strong className="min-w-0 break-words text-sm font-semibold text-neutral-900">{value || "None"}</strong>
    </div>
  );
}
    ...
    <section className="grid gap-4">
      <header className={`${panelClass} flex flex-col gap-4 md:flex-row md:items-start md:justify-between`}>
        <div className="min-w-0">
          ...
          <span className={`${ticketCodeClass} mt-3`}>{ticketCode(ticket)}</span>
          <h1 className="my-1 break-words text-3xl font-bold leading-tight text-neutral-950">{ticket.title}</h1>
        </div>
        <div className="grid w-full gap-2 sm:w-auto sm:min-w-64 md:justify-end">
          <label className={fieldClass}>
            <span>Status</span>
            <select className={selectClass} value={ticket.status} onChange={(event) => onMove(ticket.id, event.target.value as TicketStatus)}>
              {statuses.map((status) => (
          ...
          <div className="flex flex-wrap gap-2 sm:justify-end">
            {needsApproval ? (
              <ActionButton
                ...
                title={hasExecutionPlan ? "Approve this task for implementation" : "Add an execution plan before approval"}
                onAction={onAction}
              />
            ) : null}
            <button className={`${buttonClass} ${secondaryButtonClass}`} type="button" onClick={() => setIsEditing((current) => !current)}>
      ...
      <section className={panelClass}>
        <div className="mb-3 flex items-start justify-between gap-3">
          <div>
            <h2 className="m-0 text-base font-bold text-neutral-950">Implementation progress</h2>
            <p className="m-0 text-sm leading-6 text-neutral-500">Status-aware progress for the current task flow.</p>
          </div>
          <strong className="text-lg font-bold text-violet-700">{progress}%</strong>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[360px_minmax(0,1fr)]">
        <aside className="grid content-start gap-4">
          <section className={panelClass}>
            <h2 className="mb-3 text-base font-bold text-neutral-950">Properties</h2>
            ...
            <Property label="Review" value={ticket.user_reviewed ? "Approved" : "Needs review"} />
            <Property label="Parent" value={ticket.parent ? `${ticketCode(ticket.parent)} ${ticket.parent.title}` : "None"} />
            ...
            <Property label="PR" value={ticket.pr_url || ticket.pr_number || "None"} />
            ...
            {ticket.children.length ? (
              <div className="grid gap-2">
                {ticket.children.map((child) => (
                  <button className="grid min-w-0 gap-1 rounded-lg border border-neutral-200 bg-neutral-50 p-3 text-left transition hover:border-violet-200 hover:bg-violet-50" key={child.id} type="button" onClick={() => onOpen(child.id)}>
                    <span className={ticketCodeClass}>{ticketCode(child)}</span>
                    <strong className="min-w-0 truncate text-sm font-bold text-neutral-950">{child.title}</strong>
                    <span className="text-xs font-semibold text-neutral-500">{formatStatus(child.status)}</span>
                  </button>
                ))}
              </div>
            ) : (
          ...
          <section className={panelClass}>
            <h2 className="mb-3 text-base font-bold text-neutral-950">Source ticket</h2>
        ...
        <main className="grid min-w-0 gap-4">
          {isEditing ? (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h2 className="m-0 text-base font-bold text-neutral-950">Edit ticket</h2>
                  <p className="m-0 text-sm leading-6 text-neutral-500">Change ticket metadata and content.</p>
                </div>
                <button className={`${buttonClass} ${secondaryButtonClass}`} type="button" onClick={() => setIsEditing(false)}>
                  Cancel
                </button>
              </div>
              <div className="grid gap-3 xl:grid-cols-[1fr_150px_1fr_140px]">
                <label className={fieldClass}>
                  <span>Title</span>
                  <input className={inputClass} name="title" defaultValue={ticket.title} />
                </label>
                <label className={fieldClass}>
                  <span>Type</span>
                  <select className={selectClass} name="type" value={editType} onChange={(event) => setEditType(event.target.value as TicketType)}>
                    {types.map((item) => (
                ...
                <label className={fieldClass}>
                  <span>Parent</span>
                  <select className={selectClass} name="parent_id" defaultValue={ticket.parent_id || ""}>
                ...
                <label className={fieldClass}>
                  <span>Progress</span>
                  <input className={inputClass} name="progress_percent" type="number" min={0} max={100} defaultValue={ticket.progress_percent ?? ""} />
                </label>
              </div>
              ...
              <div className="grid gap-3 xl:grid-cols-3">
                <label className={fieldClass}>
                  ...
                  <input className={inputClass} name="source_type" defaultValue={ticket.source_type || ""} />
                </label>
                <label className={fieldClass}>
                  <span>Source id</span>
                  <input className={inputClass} name="source_id" defaultValue={ticket.source_id || ""} />
                </label>
                <label className={fieldClass}>
                  <span>Source URL</span>
                  <input className={inputClass} name="source_url" defaultValue={ticket.source_url || ""} />
                </label>
              </div>
              <label className={fieldClass}>
              ...
              <div className="grid gap-3 xl:grid-cols-3">
                <label className={fieldClass}>
                  <span>PR URL</span>
                  <input className={inputClass} name="pr_url" defaultValue={ticket.pr_url || ""} />
                </label>
                <label className={fieldClass}>
                  <span>PR number</span>
                  <input className={inputClass} name="pr_number" defaultValue={ticket.pr_number || ""} />
                </label>
                <label className={fieldClass}>
                  <span>PR status</span>
                  <input className={inputClass} name="pr_status" defaultValue={ticket.pr_status || ""} />
                </label>
              </div>
              <div className="grid gap-3 xl:grid-cols-2">
                <label className={fieldClass}>
                  <span>Pipeline status</span>
                  <input className={inputClass} name="pipeline_status" defaultValue={ticket.pipeline_status || ""} />
                </label>
                <label className={fieldClass}>
                  <span>Pipeline URL</span>
                  <input className={inputClass} name="pipeline_url" defaultValue={ticket.pipeline_url || ""} />
                </label>
              </div>
              <label className={fieldClass}>
          ...
          <section className={panelClass}>
            <h2 className="mb-3 text-base font-bold text-neutral-950">Git commits</h2>
                ...
                <table className="w-full min-w-[720px] border-collapse text-left text-sm">
                  ...
                  <tbody>
                    {ticket.commits.map((commit) => (
                      <tr className="border-b border-neutral-100 last:border-b-0" key={commit.id}>
                        <td className="px-3 py-2 font-mono text-xs text-violet-700">{commit.commit_hash}</td>
                        <td className="px-3 py-2 text-neutral-700">{commit.branch || "None"}</td>
                        <td className="px-3 py-2 text-neutral-700">{commit.message || "No message"}</td>
                        <td className="px-3 py-2 text-neutral-700">{commit.author || "Unknown"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <section className={panelClass}>
            <h2 className="mb-3 text-base font-bold text-neutral-950">PR and pipeline</h2>
          ...
          <section className={panelClass}>
            <h2 className="mb-3 text-base font-bold text-neutral-950">Activity history</h2>
            {ticket.events.length ? (
              <ol className="grid gap-3">
                {ticket.events
                  ...
                  .reverse()
                  .map((event) => (
                    <li className="grid grid-cols-[10px_minmax(0,1fr)] gap-3" key={event.id}>
                      ...
                      <div className="min-w-0 rounded-lg border border-neutral-200 bg-neutral-50 p-3">
                        <strong className="block break-words text-sm font-bold capitalize text-neutral-950">{event.type.replaceAll(".", " ")}</strong>
                        <p className="m-0 text-xs font-semibold text-neutral-500">
                          {event.actor} - {event.created_at}
                        </p>
                        {eventSummary(event.payload) ? <p className="m-0 mt-2 break-words text-sm leading-6 text-neutral-600">{eventSummary(event.payload)}</p> : null}
                      </div>
                    </li>
                  ))}
              </ol>
