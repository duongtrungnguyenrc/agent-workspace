...
import { statusTone, tone, typeTone } from "../lib/styles";
import { Button } from "@/components/ui/button";
import { Card } from "@/components/ui/card";
import { cn } from "@/lib/utils";
import { MinusIcon, PlusIcon, RotateCcwIcon, WaypointsIcon } from "lucide-react";
...
function GraphCard({
  ...
  const toneName = toneFor(node);
  const statusToneValue =
    node.status !== "project" ? statusTone[node.status] : "violet";
  const typeToneValue = node.ticket ? typeTone[node.ticket.type] : toneName;
  ...
  return (
    ...
    >
      <div className="p-2">
        <button
          className={cn(
            "grid h-full w-full content-start gap-2 overflow-hidden rounded-xl border border-l-[3px] bg-card p-3 text-left text-card-foreground shadow-md transition hover:-translate-y-0.5 hover:shadow-lg",
        ...
        >
          ...
          <strong className="line-clamp-2 min-w-0 text-sm font-semibold leading-5">
            {node.title}
          </strong>
          ...
          <span className="mt-auto grid grid-cols-[34px_minmax(0,1fr)] items-center gap-2">
            ...
            <span className="h-1.5 overflow-hidden rounded-full bg-muted">
              <i
                className={cn("block h-full rounded-full", tone(statusToneValue).fill)}
    ...
    <Card className="mb-4 gap-0 overflow-hidden p-0">
      <header className="flex flex-col gap-3 border-b px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <h2 className="m-0 text-sm font-semibold">Hierarchy graph</h2>
          <span className="text-xs text-muted-foreground">drag to pan, scroll to zoom</span>
        </div>
        <div className="flex flex-wrap gap-1.5">
          <Badge tone="indigo">{storyCount} groups</Badge>
          <Badge tone="fuchsia">{useCaseCount} features</Badge>
          <Badge tone="teal">{taskCount} tasks</Badge>
          <Badge tone="neutral">
            {Math.round(view.zoom * 100)}% zoom
          </Badge>
        </div>
      </header>

      <div
        className="relative h-[min(760px,calc(100vh-260px))] min-h-[560px] overflow-hidden bg-[radial-gradient(circle_at_1px_1px,var(--color-border)_1px,transparent_0)] [background-size:22px_22px] overscroll-contain"
        ref={viewportRef}
      >
        {tickets.length ? (
          <>
            <div className="absolute top-3 right-3 z-10 flex items-center gap-1 rounded-lg border bg-card p-1 shadow-sm">
              <Button
                variant="ghost"
                size="icon-sm"
                aria-label="Zoom out"
              ...
              <Button variant="ghost" size="sm" onClick={() => setView(defaultView)}>
                <RotateCcwIcon /> Reset
              </Button>
              <Button
                variant="ghost"
            ...
            >
              ...
              >
                {graph.links.map((link) => (
                  <path
                    className="fill-none stroke-border stroke-[2.5]"
